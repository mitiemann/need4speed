...
# ╔═╡ 584a6cb4-eeff-4362-ae9d-003cdbf13264
...
# ╔═╡ 8e1c1f08-0bdc-4f81-aa00-c5472c58b20e
md"""
## Alternate titles
- The reasonable effectiveness of multiple dispatch
"""
...
# ╔═╡ 00163fb8-1127-4652-8d44-5cbb55aa4a37
md"""
### Lotka-Volterra
"""

# ╔═╡ 83b3cb38-2b17-4edb-bf4e-cb7c7d8ef089
begin
	α = 1.1 ± 0.1
	β = 0.4 ± 0.05
	γ = 0.4 ± 0.01
...
# ╔═╡ 7a7c274d-665a-453c-9a01-f03f53902d9e
function lotka_volterra!(du, u, p, t)
	du[1] = α * u[1] - β * u[1] * u[2]
	du[2] = -γ * u[2] + δ * u[1] * u[2]
end

# ╔═╡ 997cfda5-2c43-4377-a0e0-240dae7c596b
begin
	prob = ODEProblem(lotka_volterra!, u₀, tspan)
	sol = solve(prob)
end

# ╔═╡ 91edad47-e743-49ea-b871-17a241715264
plot(sol, xaxis = "t", layout = (2, 1))

# ╔═╡ 7dbd4821-90c7-41c5-ae44-771efcdcd69f
md"""
### Chaotic system
Let's try the Lu Chen attractor for a change
"""

# ╔═╡ 66ac6ed1-1a70-47f4-83df-10c08f059764
begin
	a = 36.
	c = 20.
	b = 3.
...
# ╔═╡ 72c0f0fd-9ae3-4d45-89ec-0a23b11d438e
function lu_chen!(ds, s, p, t)
	ds[1] = a*(s[2] - s[1])
	ds[2] = s[1] - s[1]*s[3] + c*s[2] + u
	ds[3] = s[1]*s[2] - b*s[3]
end

# ╔═╡ 785209a2-6979-4411-bcf4-5cdf94509049
prob_lu = ODEProblem(lu_chen!, s₀, (0., 2000.))

# ╔═╡ 472e89b4-3363-4f5b-aa22-022fb875e56a
sol_lu = solve(prob_lu, Rodas5P(), reltol = 1e-8, abstol = 1e-8)

# ╔═╡ 7aef7916-075a-4e9d-9bbe-926d1d2cb866
plot(sol_lu, idxs = (1, 2, 3))

# ╔═╡ 7b82c8de-ce9f-4d46-9c8e-9e7d4afc7a19
md"""
## Running example 2: the calculator problem
[TODO]
"""

# ╔═╡ 3a755f18-5a0e-46bb-93ec-68ab813677eb
md"""
!!! info "Is there a possibility to 'disable' MD in Julia"
	Since Julia has multiple dispatch, it's almost impossible to demonstrate problem within Julia. One way around this would be to demonstrate within Jupyter noteebooks with Haskell and Python (Object-oriented). Or is there a way to disable multiple dispatch within Julia?
"""

# ╔═╡ f5d59cf8-da85-434c-8915-59b60a6a888f
abstract type Expression end

# ╔═╡ b611b9d9-59f5-4932-817e-e23591f99712
struct Value <: Expression val::Int64 end

# ╔═╡ 994a95d4-7e8b-4016-9551-c7ef26a0d1de
struct Addition <: Expression 
	left::Expression
	right::Expression
end

# ╔═╡ dd4f8281-9a43-4958-8cda-10bb51be909d
evaluate(v::Value) = v.val

# ╔═╡ 0cccc4f2-5730-48f1-af73-fdca54bc8612
evaluate(a::Addition) = evaluate(a.left) + evaluate(a.right)

# ╔═╡ 4b796b62-13f9-46e8-8871-e3375d001388
evaluate(Addition(Value(1), Value(2)))
...
# ╔═╡ 81e8d8ab-6647-42c1-9b8d-dd3a93ff14d2
begin
...
# ╔═╡ c9b9e919-6a5e-42a6-b9d7-a3ec3e572adb
function encounter(a::Pet, b::Pet)
	verb = meets(a, b)
	return "$(a.name) meets $(b.name) and $verb"
end

# ╔═╡ 5cf992df-991e-4eca-9016-82a43cfd459b
begin
	fido = Dog("Fido")
	rex = Dog("Rex")
...
# ╔═╡ c671318f-c563-4629-ab7b-555a769e4966
encounter(fido, rex)

# ╔═╡ 82b08885-69c7-439b-b4f2-739e5ebb0a89
encounter(fido, whiskers)

# ╔═╡ 051a52d7-d897-4a9c-b378-cb940b5bc3dc
encounter(whiskers, rex)

# ╔═╡ c88b6c55-1c4c-42fd-b739-a527c4ce608e
encounter(whiskers, spots)

# ╔═╡ 8a291a9d-9bbe-4f2b-9b83-18566cc66556
md"""
## Running example 4(?): the dillo problem
...
manifest_format = "2.0"
project_hash = "b7dabc06f47071004fe9eca6caeef669ee50f974"
